/**
 * Program Name: ListWindow_With_ScrollPane.java
 ...
 *           
 *           REVISION:we put the JList object inside a SUB CONTAINER called
 *           a JScrollPane. This way we can manage the screen "real estate" 
 *           and minimize the size of the JList, but still give the user access
 *           to all of the data values using scroll bars. 
 *           
 * Coder: Bill Pulling 
 * Date: July 8, 2025
 * 
...
public class ListWindow_With_ScrollPane extends JFrame
{
	...
	//REVSION #1: add the JScrollPane in class wide scope
	private JScrollPane scrollPane;
	
	//constructor
	public ListWindow_With_ScrollPane()
	{
    ...
    this.pack();//reduces screen footprint of the application to as small as possible
		//last line
	 ...
	 * REVISIONS HERE to add JScrollPane
	 */
	private void buildMonthPanel()
	{
		...
		//REVSION #2: comment out where we add a border because JScrollPane has its own border
		//monthList.setBorder(BorderFactory.createLineBorder(Color.BLACK));
		
		//REVISION #2...limit the real estate used by setting the visible row count
		// of the JList....
		monthList.setVisibleRowCount(2);//only six month names will be visible
		
		//REVISION #3: create the JScrollPane object and pass it the JList object
		scrollPane = new JScrollPane(monthList);
		//add it to the panel and comment out the line below where we added JList
		// to the monthPanel
		monthPanel.add(scrollPane);
		
		//REVSISON: comment out line below...add the Jlist object to the monthPanel
	...
	{
		// anonymous object
		new ListWindow_With_ScrollPane();
